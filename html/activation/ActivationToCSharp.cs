...
namespace webshop_backend.html.activation
{
    public class ActivationToCSharp
    {
        public static string Activation(User user, IOptions<Urls> urlSettings) {
            var body = @"<form action='{{url}}/auth/activate/{{id}}' method='get'>
    <h1>Hello {{title}} {{name}}</h1>
    <p>Click the button below to activate your account.</p>
    <button type='submit' style='display: inline-block;font-weight: 400;text-align: center;white-space: nowrap;vertical-align: middle;
